...
- **Widgets that compute, not widgets that reveal.** A widget whose every state is a string written into the asset is a slideshow with buttons — it repeats the lesson in a second font. Prefer components that scrub, compute, or respond to a changed value. Two reusable ones, both JSON-configured in the page:
  - **`assets/flow.js`** — the flow grid. Rows are values, columns are events, a zone band says where the work is. Reach for it whenever the subject is *data moving through the system*. **An event is a trigger** — a person doing something, or a function returning a value — never a keystroke, which is why every event carries `fn` and `ret`. The person stays visible: a `scenario` line, and `by: "user"` columns marked apart from code. Live in [`MAP.html`](MAP.html) §3; a lesson can carry a shorter event list.
  - **`assets/timeline.js`** — a broad-granularity scrubber (stages, phases — never line-by-line) with three tenses, where **future shows no data** because nothing has observed it yet.
- **Components restate what a host stylesheet might set.** `lesson.css` styles bare `pre`, `h3`, `table` and `th`; a component that inherits those silently renders wrong — invisible spine text, uppercased value names. Reset the properties in the component's own CSS rather than discovering each bleed one screenshot at a time. This is a class of bug no link check or JS error listener can catch; only looking at it finds it.
...
- **What lives where.** NOTES: preferences, lesson style, workspace facts, target-app examples. `learning-records/`: understandings and corrections, each tagged `Altitude: structural | mechanical`. [`MAP.html`](MAP.html): the whole topic at once — a mirror of the roadmap, never a second record of progress. Progress: [`ROADMAP.md`](ROADMAP.md) Status block — never “P1.x is done” here.
