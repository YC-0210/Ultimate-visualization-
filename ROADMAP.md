...
> `supervisualizer/` is the **real tool**, built by hand, that runs on the developer's own machine and watches a live backend.
...
- [x] **P1.2** Capture the resolved route (`request.resolver_match` — pattern, url_name, view class, kwargs).
- [x] **P1.3** Capture post-middleware state: `request.user`, session keys, auth.
- [x] **P1.4** Capture view facts: class, `permission_classes`, `serializer_class`, queryset model.
- [x] **P1.5** Capture SQL with `connection.execute_wrapper` — P0.4a confirmed OTel gives no DB spans for Django's ORM, so this is ours to build. (`connection.queries` also only populates when `DEBUG=True`.)
- [x] **P1.6** Capture the response: status, headers, body, size.
- [x] **P1.7** 🔴 **Probe the serializer.** Capture `validated_data` *and the types inside it* — the string-becomes-model-instance moment, the single most valuable thing the tool shows. **P0.4a already proved the approach**: monkeypatch `Serializer.is_valid` / `BaseSerializer.save`, wrap each in a span, JSON-encode the values. Start from `spikes/P0.4a-code/02_probe_integration.py`, which does exactly this against a real request.
- [x] **P1.8** Capture template render + context for non-DRF views.
- [x] **P1.9** Serialise everything to the Trace schema and write it to a file per request. → `POST /api/cart/item/` writes five stages (`render_output` correctly absent) plus a flat `sql` list, to `BASE_DIR/supervisualizer-traces/<utc-timestamp>-<trace_id[:8]>.json`. **Phase 1's "Done when" is met.** Two threads left open deliberately: `Cookie` is unredacted, and nothing emits `mutate_data` even on a writing request.
- [x] **P1.10** 🔴 Make capture safe: never let a probe crash a real request. Wrap every hook in try/except and drop the trace on failure rather than 500ing the app. → `/media/menu/images.jpeg` is 200 (`response.streaming`); a forced `encode` raise leaves the page up and writes `capture_errors`.
